...
# Load environment variables and initialize agent
load_dotenv("./.env")
...
@app.post("/query-documents")
async def query_documents(request: QueryRequest):
    global idp_agent, collection, processor
    try:
        if not collection:
            raise HTTPException(status_code=500, detail="ChromaDB not available")
        ...
        # If Gemini is available and requested, use it to generate a comprehensive answer
        if request.use_llm_for_answer:
            # Use IDP agent to process query
            response = await idp_agent.arun(f"{{'query': '{request.query}', 'documents': {documents}}}")
            ...
            return {
                ...
                "answer": answer,
                "sources": documents,
                "method": "llm_processed"
            }
        else:
            # If no relevant documents found, return a default response
            return {
                "query": request.query,
                "answer": "Listed below are the relevant documents",
                "sources": [documents],
                "method": "no_llm"
            }
...
async def query_documents_stream(request: StreamQueryRequest):
    """Stream agent responses with tool calls and thinking"""
    global idp_agent, collection, processor
    
    async def generate_stream():
        try:
            if not collection:
                yield f"data: {json.dumps({'type': 'error', 'content': 'ChromaDB not available'})}\n\n"
                return
            ...
            if not results['documents'] or not results['documents'][0]:
                yield f"data: {json.dumps({'type': 'final', 'content': 'No relevant documents found.'})}\n\n"
                return
            ...
            # Send sources first
            yield f"data: {json.dumps({'type': 'sources', 'content': documents})}\n\n"
            
            # Stream agent response
            yield f"data: {json.dumps({'type': 'status', 'content': 'Agent is thinking...'})}\n\n"
            
            # Use IDP agent with streaming
            response_stream = await idp_agent.arun(
                f"{{'query': '{request.query}', 'documents': {documents}}}",
                stream=True,
                stream_intermediate_steps=True
            )
            
            # Handle the async iterator properly
            if hasattr(response_stream, '__aiter__'):
                async for event in response_stream:
                    ...
                    if event.event == "RunResponseContent":
                        ...
                        event_data['content'] = f"🔧 Using tool: {getattr(event, 'tool', 'Unknown tool')}"
                    elif event.event == "ReasoningStep":
                        event_data['content'] = f"💭 {getattr(event, 'content', '')}"
                    elif event.event == "ToolCallCompleted":
                        event_data['content'] = f"✅ Tool completed"
                    elif event.event == "RunCompleted":
                        event_data['type'] = 'complete'
                        event_data['content'] = 'Response completed'
                    elif event.event == "RunError":
                        event_data['type'] = 'error'
                        event_data['content'] = f"Error: {getattr(event, 'content', 'Unknown error')}"
                    else:
                        # Skip other events or include them based on your needs
                        continue
                    
                    yield f"data: {json.dumps(event_data)}\n\n"
            else:
                # Fallback: if it's not an async iterator, treat as regular response
                yield f"data: {json.dumps({'type': 'RunResponseContent', 'content': str(response_stream)})}\n\n"
